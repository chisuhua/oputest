...
class __static_size_thread_block_tile_base
{
    friend class details::_coalesced_group_data_access;

#ifdef _CG_CPP11_FEATURES
    static_assert(Size == 1 || Size == 2 || Size == 4 || Size == 8 || Size == 16 || Size == 32,
                  "Size must be one of 1/2/4/8/16/32");
#else
...
public:
    ...
    _CG_STATIC_QUALIFIER unsigned int thread_rank() {
        return (details::laneid() & th::laneMask);
    }
...
protected:
    _CG_QUALIFIER thread_block_tile() { };
...
public:
    // Convert to parent-type-erased thread_block_tile
    _CG_QUALIFIER operator thread_block_tile<Size, void>() const {
        return thread_block_tile<Size, void>(staticParentBaseT::meta_group_rank(), staticParentBaseT::meta_group_size());
    }
};
    ...
    public thread_group_base<coalesced_group_id>
{
    ...
    template <unsigned int, typename ParentT> friend class thread_block_tile;
    friend class details::_coalesced_group_data_access;

    typedef __static_size_thread_block_tile_base<numThreads> staticSizeBaseT;

protected:
    _CG_QUALIFIER thread_block_tile(unsigned int meta_group_rank, unsigned int meta_group_size) {
...
namespace details {
    ...
    template <unsigned int Size, typename ParentT>
    struct tiled_partition_impl : public thread_block_tile<Size, ParentT> {
        _CG_QUALIFIER tiled_partition_impl(const ParentT&) : thread_block_tile<Size, ParentT>() {}
    };
...
#endif
        _CG_QUALIFIER tiled_partition_impl(thread_block_tile<ParentSize, GrandParent> const &) :
            thread_block_tile<Size, thread_block_tile<ParentSize, GrandParent> >() {}
    };
};
